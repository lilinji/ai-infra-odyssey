...
categories: ["AI_Infra大话西游之水滴石穿", "CUDA与算子性能优化_M2"]
math: true
showToc: true
author: "Ringi"
---
...
> **知识底账索引**：
>
> - 核心理论证据：[FlashAttention 深度剖析（AI-fundamentals）](file:///d:/GeneTind/Interview/AI_BOOK/AI-fundamentals/09_inference_system/vllm/module_analysis/flashattention_deep_dive.md)
> - 原理与公式证明：[FlashAttention V1 详解（AIInfraGuide）](file:///d:/GeneTind/Interview/AI_BOOK/AIInfraGuide/docs/guides/模块二-CUDA编程与算子优化/6.1-FlashAttention%20V1详解.md)
> - 经典算子源码：[LeetCUDA 算子实战（LeetCUDA）](file:///d:/GeneTind/Interview/AI_BOOK/LeetCUDA/kernels/)
...
| **02** | “分块尺寸（BLOCK_M, BLOCK_N）配得越小，SRAM 占用越低，性能越高。”       | **错误**。分块太小会导致外层循环次数剧增，不仅每次 GEMM 无法打满 Tensor Core 的大矩阵乘模式，而且反复加载 $Q$ 的 HBM 流量也会成倍放大。     | 在 A100 上通常将 $BLOCK\_M=64, BLOCK\_N=64$ 或 $128 \times 64$ 作为黄金基准，利用 Triton Autotune 寻找最优解。                    |
...
2. **本地核心知识库代码对照（AI_BOOK）**：
   - 理论与演进：[vLLM FlashAttention 模块深度分析（AI-fundamentals）](file:///d:/GeneTind/Interview/AI_BOOK/AI-fundamentals/09_inference_system/vllm/module_analysis/flashattention_deep_dive.md)
   - 算法手算推导：[FlashAttention V1 详解与算法演进（AIInfraGuide）](file:///d:/GeneTind/Interview/AI_BOOK/AIInfraGuide/docs/guides/模块二-CUDA编程与算子优化/6.1-FlashAttention%20V1详解.md)
   - 算子实战实现：[LeetCUDA Attention 优化源码（LeetCUDA）](file:///d:/GeneTind/Interview/AI_BOOK/LeetCUDA/kernels/)
3. **官方开源项目与核心代码实现**：
