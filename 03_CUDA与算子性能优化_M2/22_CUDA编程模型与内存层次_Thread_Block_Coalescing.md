...
1. **CUDA 内存模型与合并访存权威专著**：
   - 深入学习合并访问判定与 AoS/SoA 决策：参考 [1.3-CUDA内存模型.md](file:///d:/GeneTind/Interview/AI_BOOK/AIInfraGuide/docs/guides/模块二-CUDA编程与算子优化/1.3-CUDA内存模型.md) 与 [2.2-内存访问优化.md](file:///d:/GeneTind/Interview/AI_BOOK/AIInfraGuide/docs/guides/模块二-CUDA编程与算子优化/2.2-内存访问优化.md)。
2. **Bank Conflict 真实物理压测基准**：
   - A100 实测 `gcd(stride, 32)` 与 转置 Padding 提升基准源码：参见 [13_bank_conflict_bench.cu](file:///d:/GeneTind/Interview/AI_BOOK/AI-fundamentals/02_gpu_programming/02_cuda/code/13_bank_conflict_bench.cu)。
3. **Occupancy 与资源分配计算器**：
   - 寄存器分配粒度（256 Regs）、共享内存与 Little's Law 延迟隐藏推导：参见 [2.3-Occupancy与资源分配.md](file:///d:/GeneTind/Interview/AI_BOOK/AIInfraGuide/docs/guides/模块二-CUDA编程与算子优化/2.3-Occupancy与资源分配.md)。
4. **工业级 GEMM 算子中的共享内存冲突消除**：
   - LeetCUDA 高性能 SGEMM 与 FlashAttention 共享内存 Swizzle 实战：参见 [LeetCUDA/kernels/sgemm/README.md](file:///d:/GeneTind/Interview/AI_BOOK/LeetCUDA/kernels/sgemm/README.md)。
5. **NVIDIA 官方体系结构白皮书**：
