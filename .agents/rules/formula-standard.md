---
description: AI-Infra Odyssey 数学公式排版与 KaTeX 兼容性规范
globs: **/*.md
---

# AI-Infra Odyssey 数学公式排版与 KaTeX 规范

所有在本项目中编写、修改、生成的 Markdown 教学文档与技术 RFC，必须严格遵守以下 KaTeX 数学公式排版规范。
...
- **【绝对禁止】** 严禁在 `\text{...}`、`\mathrm{...}`、`\mathbf{...}` 内部出现裸下划线 `_` 或转义下划线 `\_` / `\\_`。
  ...
  - 原因：在 KaTeX 文本模式下，`\\` 被解析为断行符，紧随其后的 `_` 缺少合法数学基底，直接导致 `ParseError: KaTeX parse error: Expected 'EOF', got '_'` 语法崩溃。
- **【推荐规范】**
...
- **【禁止】** `\mathbf{16\text{ 字节}}`、`\mathbf{20\text{ 步}}`、`\mathbf{0.31\text{ 秒}}`
  - 原因：部分 Markdown/KaTeX 渲染引擎（如 VS Code 内置预览、Obsidian 移动端、GitHub Web）对 `\mathbf{}` 内多字节字符集的字体回退支持不一致，会导致字符丢失或排版错位。
...
## 核心准则四：统一体系结构与排队论量纲符号（Unified System Symbols）
...
## 核心准则五：GitHub GFM 解析器特异性避坑（GFM MathJax Parsing Compatibility）
- **【标点空格避让】**：行内公式紧贴中文全角标点（如冒号 `：`）时，GFM 会因缺乏空白字符而将 `$` 当作普通字符忽略。中文标点后必须保留半角空格：
  - 错误：`：$2 \times P$` $\to$ **正确**：`： $2 \times P$`（或使用带反引号定界符 `：$`2 \times P`$`）
- **【严禁列表项内塞入块级公式】**：`$$` 块严禁无空行直接挂在 `- 列表项：` 后面或同行，否则 GFM 解析器会强行插入 `<br>` 并将 `_` 错误解析为 Markdown 斜体标签 `<em>`，导致整段公式渲染崩溃。必须采用独立小标题配合段落级 `$$`（前后空行），或者使用 ```` ```math ```` 语法块。
