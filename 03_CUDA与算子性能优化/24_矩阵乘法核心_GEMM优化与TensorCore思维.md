...
$$
I_{\text{tiled}} = \frac{2 \cdot BM \cdot BN \cdot BK}{4 \cdot BK \cdot (BM + BN)} = \frac{BM \cdot BN}{2 \cdot (BM + BN)}
$$
...
$$
I_{\text{tiled}} = \frac{128 \times 128}{2 \times (128 + 128)} = \frac{16384}{512} = \mathbf{32.0 \text{ FLOPs/Byte}}
$$
...
- 但如果将 $A$ 的 $8 \times 1$ 列向量与 $B$ 的 $1 \times 8$ 行向量做**外积（Outer Product）**：
  ...
  $$
  C_{\text{accum}}[i][j] += r_a[i] \times r_b[j], \quad \forall i \in [0, 7], j \in [0, 7]
  $$
