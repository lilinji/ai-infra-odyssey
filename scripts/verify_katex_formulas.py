#!/usr/bin/env python3
"""
AI-Infra Odyssey KaTeX Formula Compliance Linter
Scans all Markdown files in the repository to ensure 100% KaTeX compatibility
and compliance with FORMULA_SPECIFICATION.md.
"""
...
def scan_markdown_file(file_path):
    ...
    for lno, line in enumerate(lines, 1):
        ...
        # Track code blocks (ignore formulas inside code blocks)
...
def main():
    ...
    print("=" * 70)
    print("AI-Infra Odyssey KaTeX Formula Compliance Linter")
    print("=" * 70)
    ...
    for d in TARGET_DIRS:
        ...
        if dir_violations == 0:
            print(f"✅ [{d}] Passed cleanly.")
            
    print("\n" + "=" * 70)
    print(f"Summary: {scanned_files} files scanned across {modules_scanned} volumes.")
    if total_violations == 0:
        print("🎉 SUCCESS: 0 KaTeX formula violations found! All clean.")
        print("=" * 70)
        sys.exit(0)
    else:
        print(f"⚠️  FAILED: Found {total_violations} total violation(s).")
        print("=" * 70)
        sys.exit(1)
