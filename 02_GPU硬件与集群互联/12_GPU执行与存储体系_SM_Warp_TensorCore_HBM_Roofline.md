...
![Ringi 导师解构：GPU 核心架构与 Roofline 性能工坊全景](assets/ringi_01_gpu_arch_roofline_overview.png)
...
![现代 GPU 微架构、分级存储体系与 Roofline 性能边界全景图](assets/arch_01_gpu_sm_memory_roofline.svg)
...
![Ringi 导师解构：SM 子核心分区与 Warp 零开销调度工作流](assets/ringi_01_sm_warp_scheduler_stall.png)
...
| 4. Stall Barrier       | 线程块内部执行了 `__syncthreads()` 显式屏障等待 快的 Warp 必须挂起等待最慢的一个 Warp 抵达             | Block 内不同 Warp 执行速度严重失衡; 需减少无谓同步，拆解大块算子             |
...
![Ringi 导师解构：GPU 存储金字塔与 128 字节合并访存车道全景](assets/ringi_01_storage_pyramid_coalescing.png)
...
![Ringi 导师解构：Roofline 性能物理模型几何判决与算术强度翻越](assets/ringi_01_roofline_model_ai_cliff.png)
...
>
>      $$
>      T_{\text{min}} = \frac{140\text{ GB}}{3.35\text{ TB/s}} = \frac{140 \times 10^9}{3.35 \times 10^{12}} \approx \mathbf{0.0418 \text{ s}}（约 0.0418 秒） \quad (41.8\text{ ms})
>      $$
>
>    - 理论最高生成吞吐：
>
...
>
>      $$
>      N_{\text{warps}} = \frac{400\text{ Cycles}}{1\text{ Cycle/Warp}} = \mathbf{400 \text{ Warps}}（400 个 Warp）
>      $$
>
> 3. **指令级并行（ILP）的分摊**：
