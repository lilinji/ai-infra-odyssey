...
- [4. 通算重叠（Overlap）的残酷真相：SM 竞争与 k 倍率膨胀](#4-通算重叠overlap的残酷真相sm-竞争与-k-倍率膨胀)
  - [4.1 通算重叠不是免费的午餐：通信与计算同跑时，耗时发生的乘性膨胀 $t_{\text{comm-overlap}} = t_{\text{comm-solo}} \times k$](#41-通算重叠不是免费的午餐通信与计算同跑时耗时发生的乘性膨胀-t_comm_overlap--t_comm_solo-times-k)
...
## 4.1 通算重叠不是免费的午餐：通信与计算同跑时，耗时发生的乘性膨胀 $t_{\text{comm-overlap}} = t_{\text{comm-solo}} \times k$
...
$$
t_{\text{comm-overlap}} = t_{\text{comm-solo}} \times k \quad (k > 1.0)
$$
